  ...
  <Dialog v-model="show" :options="{ size: 'xl' }">
    <template #body>
      <div class="px-4 pt-5 pb-6 bg-surface-modal sm:px-6">
        ...
        <FieldLayout v-if="tabs.data?.length" :tabs="tabs.data" :data="savingGroup.doc" doctype="CRM Organization" />
        <ErrorMessage class="mt-8" v-if="error" :message="__(error)" />
      </div>
      <div class="px-4 pt-4 pb-7 sm:px-6">
        <div class="space-y-2">
          <Button class="w-full" variant="solid" :label="__('Create')" :loading="loading" @click="createSavingGroup" />
        </div>
      </div>
    </template>
  </Dialog>
...
const { document: savingGroup, triggerOnBeforeCreate } =
  useDocument('CRM Organization')

async function createSavingGroup() {
  loading.value = true
  error.value = null
  ...
  if (doc.name) {
    ...
    handleSavingGroupUpdate(doc)
  }
}

function handleSavingGroupUpdate(doc) {
  if (doc.name && props.options.redirect) {
    router.push({
      name: 'SavingGroup',
      params: { savingGroupId: doc.name },
    })
  }
  show.value = false
          ...
          column.fields.forEach((field) => {
            if (field.fieldname == 'address') {
              field.create = (value, close) => {
                savingGroup.doc.address = value
                openAddressModal()
                close()
              }
              field.edit = (address) => openAddressModal(address)
            } else if (field.fieldtype === 'Table') {
              savingGroup.doc[field.fieldname] = []
            }
          })
        })
      })
...
onMounted(() => {
  savingGroup.doc = { no_of_employees: '1-10' }
  Object.assign(savingGroup.doc, props.data)
})
