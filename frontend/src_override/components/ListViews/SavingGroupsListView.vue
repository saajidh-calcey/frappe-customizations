<template>
  <ListView :columns="columns" :rows="rows" :options="{
    ...
    <ListRows class="mx-3 sm:mx-5" :rows="rows" v-slot="{ idx, column, item }" doctype="CRM Organization">
      <ListRowItem :item="item" :align="column.align">
        ...
        <template #default="{ label }">
          ...
          <div v-else class="truncate text-base" @click="
            (event) =>
              emit('applyFilter', {
                event,
                idx,
  ...
  <ListBulkActions ref="listBulkActionsRef" v-model="list" doctype="CRM Organization" :options="{
    hideAssign: true,
  }" />
</template>
<script setup>
