...
const sections = createResource({
  ...
  auto: true,
  transform: (data) => {
    // Inject Custom Location section if it doesn't exist
    // This logic ensures the map field is visible even if not configured in the database
    let locationField = {
      label: 'Location',
    ...
    const READ_ONLY_MAP = true
    locationField.read_only = READ_ONLY_MAP

    let hasLocation = false
    data.forEach(section => {
      section.columns.forEach(col => {
        if (col.fields.find(f => f.fieldname === 'custom_location')) {
          hasLocation = true
        }
      })
    })
