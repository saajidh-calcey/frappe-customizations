# Copyright (c) 2025, saajidh and contributors
...
class LeadDupe(Document):
	...
	def assign_agent(self, agent):
		...
		assign({"assign_to": [agent], "doctype": "Lead Dupe", "name": self.name})
...
def convert_to_deal(lead, doc=None, deal=None, existing_contact=None, existing_organization=None):
	if not (doc and doc.flags.get("ignore_permissions")) and not frappe.has_permission(
		"Lead Dupe", "write", lead
	):
		frappe.throw(_("Not allowed to convert Lead to Deal"), frappe.PermissionError)

	lead = frappe.get_cached_doc("Lead Dupe", lead)
